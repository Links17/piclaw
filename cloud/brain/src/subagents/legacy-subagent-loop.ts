/**
 * Legacy subagent loops — mock / no-kernel fallback (explore, plan, coding).
 */
...
export async function runLegacyExploreLoop(
  sessionId: string,
  runId: string,
  prompt: string,
...
export async function runLegacyCodingLoop(
  sessionId: string,
  runId: string,
  task: string,
  constraints?: string,
  ...
  await store.insertSubagentMessage(runId, "user", task);
  let messages: OpenAiMessage[] = [
    { role: "system", content: systemPrompt },
  ...
  return {
    summary: finalText,
    artifacts: extractArtifacts(finalText, task),
    usage: totalUsage,
    toolCount,
  };
}

export async function runLegacySubagentLoop(
  sessionId: string,
  runId: string,
  agentType: SubagentType,
  prompt: string,
  options: { maxTurns?: number; constraints?: string } = {},
): Promise<{ summary: string; artifacts: string[]; usage: LlmUsage; toolCount: number }> {
  if (agentType === "explore" || agentType === "plan") {
    return runLegacyExploreLoop(sessionId, runId, prompt, {
      agentType,
      maxTurns: options.maxTurns,
      constraints: options.constraints,
    });
  }
  return runLegacyCodingLoop(sessionId, runId, prompt, options.constraints);
}
