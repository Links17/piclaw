...
import { getTimeline } from "../web-adapter.ts";
...
async function exportRecordingResponse(id: string, req: Request): Promise<Response> {
  const recording = await getSessionRecording(id);
...
export async function handleSessionRecordingRoutes(
  ...
  if (req.method === "GET" && pathname === "/agent/recordings") {
    return json({
      ok: true,
      recordings: await listSessionRecordings(),
      active: await listActiveSessionRecordings(),
    });
  }
  ...
  if (req.method === "POST" && pathname === "/agent/recordings/start") {
    ...
    if (includeSnapshot && meta.eventCount <= 1) {
      ...
      const snapshot = await getTimeline(String(chatJid || meta.chatJid), limit);
    ...
    return json({ ok: true, recording: (await getActiveSessionRecording(meta.chatJid)) || meta }, 201);
  }
  ...
  if (req.method === "POST" && pathname === "/agent/recordings/stop") {
    ...
    const meta = await stopSessionRecording(key);
  ...
  if (req.method === "GET" && pathname === "/agent/recordings/active") {
    ...
    return json({ ok: true, recording: await getActiveSessionRecording(chatJid) });
  }
  ...
  if (req.method === "GET" && pathname.endsWith("/export")) {
    return exportRecordingResponse(id, req);
  }
  ...
  if (req.method === "DELETE") {
    return json({ ok: true, deleted: await deleteSessionRecording(id) });
  }
